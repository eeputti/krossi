-- koutsi_bulk_setup always materialized a full year (364 days) of weekly sessions for
-- every group it created. Combined with a group's extra weekly slots (added client-side
-- right after this RPC returns, each also generating a year of sessions) a coach running
-- the onboarding wizard for several groups could flood Treenit/kalenteri with hundreds of
-- rows in one go. Let the caller say how many weeks ahead to materialize instead, capped
-- at a year and defaulting to a quarter (12 weeks) — comfortably fewer than before, and
-- adjustable per run from the wizard.
