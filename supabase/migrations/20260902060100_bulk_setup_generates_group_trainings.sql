-- A group created through the guided setup used to sit on the calendar with nothing on
-- it until a coach separately added a training and marked it "toistuu viikoittain" — the
-- exact thing the wizard already asked for (day, time, now duration) but never acted on.
-- This replaces koutsi_bulk_setup so a newly created group also materializes its first
-- year of weekly sessions into koutsi_trainings, in the same transaction as the group
-- itself. Reused (existing_id) groups are left untouched — their calendar is already
-- whatever the coach has made it.
-- Supersedes the version from supabase/migrations/20260823142228_add_coach_bulk_setup.sql.
...
-- The function deliberately performs the same tightly-scoped privileged insert as
-- koutsi_create_player: every call re-checks the signed-in user and target coach.
revoke all on function public.koutsi_bulk_setup(jsonb, uuid) from public, anon;
grant execute on function public.koutsi_bulk_setup(jsonb, uuid) to authenticated, service_role;
