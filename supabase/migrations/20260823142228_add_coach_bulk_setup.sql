-- One guided setup can create placeholder players, create or reuse groups, connect the
-- players to those groups and save weekly themes. A PostgREST RPC runs in one database
-- transaction, so a validation or insert failure rolls the whole setup back.
-- Production migration version: 20260823142228.
...
begin
  ...
  loop
    ...
    if v_existing_id is not null then
      ...
      insert into public.koutsi_groups (coach_id, name, level, weekday, time)
      values (v_coach, v_name, v_level, v_day, v_time)
