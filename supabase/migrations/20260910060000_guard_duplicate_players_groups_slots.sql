-- The coach app already blocks an exact-duplicate player name at the UI layer
-- (commit "Block creating or renaming a player into an exact duplicate name"), but every
-- insert path goes through these SECURITY DEFINER RPCs, which had no equivalent check of
-- their own. A direct RPC call, or two requests racing each other, could still create the
-- same "one real person as two koutsi_students rows" bug the UI guard exists to prevent.
-- Mirror the same guard here so the database — not just the UI — is the backstop.

create or replace function public.koutsi_create_player(name_input text, age_input integer DEFAULT NULL::integer, level_input text DEFAULT NULL::text, coach_id_input uuid DEFAULT NULL::uuid)
 returns uuid
 language plpgsql
 security definer
 set search_path to ''
...
declare
  ...
  v_id uuid;
  v_name text := btrim(coalesce(name_input, ''));
begin
  ...
  if exists (
    select 1
      ...
      join public.koutsi_students s on s.id = cs.student_id
     where cs.coach_id = v_coach
       and cs.ended_at is null
       and lower(s.display_name) = lower(v_name)
  ) then
    raise exception 'Sinulla on jo tismalleen tämän niminen oppilas';
  end if;

  v_id := gen_random_uuid();
  insert into public.koutsi_students (id, display_name, placeholder_coach_id, age, level)
  values (v_id, v_name, v_coach, age_input, level_input);
...
create or replace function public.koutsi_bulk_setup(payload jsonb, coach_id_input uuid DEFAULT NULL::uuid)
 returns jsonb
 language plpgsql
 security definer
 set search_path to ''
...
declare
  ...
  v_seen_player_names jsonb := '[]'::jsonb;
...
begin
  ...
  begin
    v_weeks_ahead := nullif(payload ->> 'weeks_ahead', '')::integer;
  exception when invalid_text_representation then
    v_weeks_ahead := null;
  end;
  v_weeks_ahead := greatest(1, least(52, coalesce(v_weeks_ahead, 12)));
  ...
  loop
    ...
    if v_existing_id is not null then
      ...
      -- Deliberately no duplicate-name guard here, unlike players below: live data shows
      -- coaches legitimately reusing a name for two time slots of the same class (e.g. two
      -- "keskiviikko aikuiset" groups at different times), so a same-coach/same-name row is
      -- not on its own evidence of the one-real-person-twice bug players can hit.
      ...
      -- Materialize the group's weekly sessions right away, starting from the next
      -- occurrence of its weekday, so it shows up on the calendar immediately —
      -- v_weeks_ahead weeks of them, not a full year.
      v_dow := case v_day
      ...
      select v_coach, v_group_id, d::date, v_time, 'Ryhmätreeni', v_duration_minutes, v_series_id
        from generate_series(v_first_date, v_first_date + (v_weeks_ahead - 1) * interval '7 days', interval '7 days') as d;
    end if;
  ...
  loop
    ...
    if v_seen_player_names ? v_norm_name then
      raise exception 'Samanniminen pelaaja on tuonnissa kahdesti';
    end if;
    if exists (
      select 1
        from public.koutsi_coach_students cs
       ...
       where cs.coach_id = v_coach
         and cs.ended_at is null
         and lower(s.display_name) = v_norm_name
    ) then
      raise exception 'Sinulla on jo tismalleen tämän niminen oppilas — valitse hänet olemassa olevana sen sijaan';
    end if;
    v_seen_player_names := v_seen_player_names || to_jsonb(v_norm_name);
...
-- koutsi_merge_students already locked the row being removed (`for update`) but not the
-- row being kept, so two concurrent merges targeting the same keep_student_id from
-- different remove_student_ids weren't serialized against each other. Lock both sides.
create or replace function public.koutsi_merge_students(coach_id_input uuid, keep_student_id uuid, remove_student_id uuid)
 returns jsonb
 language plpgsql
 security definer
 set search_path to ''
...
begin
  ...
  perform 1 from public.koutsi_students where id = keep_student_id for update;
...
-- No group has two slots on the same weekday at the same time today, so this is safe to
-- add now — it closes the double-click "Lisää harjoitusaika" race (plain insert, no RPC,
-- so there's no function-level guard to add instead).
create unique index if not exists koutsi_group_slots_unique_slot
  on public.koutsi_group_slots (group_id, weekday, "time");
