...
const KOUTSI_STORE_KEY = 'krossiKoutsiDemoV10';
...
function koutsiHydrateStudents(seed) {
  ...
  seed.students.forEach((s) => {
    ...
    s.homework = (s.homework || []).map((h, i) => ({ id: i, at: koutsiOffsetIsoStr(-h.daysAgo, 18), text: h.text, done: h.done }));
...
function koutsiLoadState() {
  ...
  return koutsiHydrateStudents(seed);
}
...
function koutsiBuildSeedMoods() {
  return {
    0: [
      ...
      { id: 1, at: koutsiOffsetIsoStr(-20, 19), score: 2, note: 'Nilkka jomotti, en uskaltanut liikkua täysillä.' },
...
function koutsiBuildSeedGoalHistory() {
  return {
    0: [
      { id: 2, at: koutsiOffsetIsoStr(-9, 20), goal: 'Varmempi kakkossyöttö ja rohkeampi verkkopeli', previousGoal: 'Kakkossyöttö sisään paineen alla', byPlayer: true },
      { id: 1, at: koutsiOffsetIsoStr(-74, 19), goal: 'Kakkossyöttö sisään paineen alla', previousGoal: 'Syötön perustekniikka kuntoon', byPlayer: true },
      { id: 0, at: koutsiOffsetIsoStr(-165, 18), goal: 'Syötön perustekniikka kuntoon', previousGoal: '', byPlayer: true },
    ],
    1: [
      { id: 1, at: koutsiOffsetIsoStr(-31, 17), goal: 'Backhandin tasapaino pitkissä vaihdoissa', previousGoal: 'Backhand kahdella kädellä haltuun', byPlayer: true },
      { id: 0, at: koutsiOffsetIsoStr(-140, 18), goal: 'Backhand kahdella kädellä haltuun', previousGoal: '', byPlayer: true },
    ],
    3: [
      { id: 1, at: koutsiOffsetIsoStr(-22, 21), goal: 'Nelinpelitaktiikan syventäminen', previousGoal: 'Verkkopeli rohkeammaksi nelinpelissä', byPlayer: true },
      { id: 0, at: koutsiOffsetIsoStr(-120, 18), goal: 'Verkkopeli rohkeammaksi nelinpelissä', previousGoal: '', byPlayer: true },
    ],
  };
}
