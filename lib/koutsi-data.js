...
const KOUTSI_PROFILE_AGE_LABELS = {
  alle20: 'Alle 20', '20-30': '20–30', '30-40': '30–40',
  '40-50': '40–50', '50-60': '50–60', '60+': '60+',
};
...
function koutsiStudentAge(studentRow, profileRow) {
  ...
  const range = KOUTSI_PROFILE_AGE_LABELS[value] ? value : null;
  return {
    age: exact,
    ageRange: range,
    ageValue: value,
    ageLabel: exact == null ? (KOUTSI_PROFILE_AGE_LABELS[range] || '') : `${exact} v`,
  };
}
...
function koutsiMapStudent(studentRow, profileRow, diaryRows, homeworkRows, videoRows, moodRows, matchNoteRows, historyRows) {
  ...
  return {
    ...
    pilotAgeGroup: studentRow.pilot_age_group || 'adult',
...
async function koutsiFetchStudentRow(uid) {
  const { data, error } = await koutsiSupabase.from('koutsi_students').select('id, pilot_age_group').eq('id', uid).maybeSingle();
  if (error) throw error;
  return data;
}
...
const KOUTSI_ERROR_PATTERNS = [
  ...
  [/valitse valmentajan vahvistama ikäryhmä/i, 'Valitse sama ikäryhmä, jonka valmentajasi vahvisti kutsua tehdessään.'],
...
async function koutsiCreatePlayer(name, age, level, coachId = null, ageGroup = 'adult', minorNoticeConfirmed = false, guardianApproved = false) {
  ...
  const { data, error } = await koutsiSupabase.rpc('koutsi_create_player_v2', {
    ...
    coach_id_input: coachId || null,
    age_group_input: ageGroup,
    minor_notice_confirmed_input: Boolean(minorNoticeConfirmed),
    guardian_approved_input: Boolean(guardianApproved),
  });
  if (error) throw error;
  return data;
}
...
function koutsiInviteMessage(code, coachName, groupName, studentId = null, ageGroup = 'adult') {
  const who = coachName ? `${coachName} täällä` : 'Moi';
  const where = groupName ? ` ryhmään ${groupName}` : '';
  const junior = ageGroup !== 'adult';
  const ageNote = ageGroup === 'child_under_13'
    ? 'Alle 13-vuotiaan tilin aktivointi edellyttää, että huoltaja on hyväksynyt käytön.'
    : junior
      ? 'Koutsi näyttää juniorille vain valmennukseen liittyvät tiedot, eikä profiili ole julkinen.'
      : 'Pelaaja vahvistaa käyttöönotossa olevansa vähintään 18-vuotias.';
  return `${who}! Otetaan Krossi Koutsi käyttöön — sieltä näet treenit, kotiläksyt ja palautteet. ${ageNote} Koutsiin ei kirjata terveystietoja.\n\nLiity${where} henkilökohtaisesta linkistä: ${koutsiInviteLink(code, studentId)}${studentId ? '' : `\n\n(Jos linkki ei aukea, mene osoitteeseen koutsi.krossi.app/pelaaja ja syötä koodi ${code}.)`}`;
}
...
// Pilotissa pelaaja voi päivittää vain ikänsä. Jaettu profiili säilyttää joko tarkan iän
// tai ikähaarukan; oppilasrivillä on tarkka ikä vain silloin, kun pelaaja on antanut sen.
