...
// training does. One year ahead, same safety cap as koutsiWeeklyDates.
async function koutsiGenerateGroupTrainings(groupId, coachId, day, time, durationMinutes, fromDate) {
  const first = koutsiNextWeekday(fromDate || koutsiTodayStr(), day);
  if (!first || !time) return { count: 0, seriesId: null };
  const dates = koutsiWeeklyDates(first, koutsiAddDays(first, 364));
...
async function koutsiAddGroupSlot({ groupId, coachId, day, time, durationMinutes }) {
  ...
  const { data, error } = await koutsiSupabase.from('koutsi_group_slots')
    ...
    .select('id').single();
  if (error) throw error;
  const { seriesId } = await koutsiGenerateGroupTrainings(groupId, coachId, day, roundedTime, duration);
  if (seriesId) {
...
// ── groups ───────────────────────────────────────────────────
async function koutsiCreateGroup({ coachId, name, level, day, time, durationMinutes, memberIds }) {
  ...
  await koutsiGenerateGroupTrainings(data.id, coachId, day, time, duration);
  return data.id;
}
...
async function koutsiUpdateGroup(groupId, { name, level, day, time, durationMinutes }) {
  const duration = durationMinutes || 60;
  ...
  if (scheduleChanged) {
    ...
    if (delErr) throw delErr;
    await koutsiGenerateGroupTrainings(groupId, existing.coach_id, day, time, duration, today);
  }
}
