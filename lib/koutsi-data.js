...
// A player's own logged tournaments — the only koutsi_trainings rows a match note's
// tournament_id is allowed to point at. Newest first, so the tournament a player is
// most likely mid-way through sits at the top of the picker in MatchNoteModal.
function koutsiTournamentsForStudent(state, studentId) {
  return koutsiTrainingsForStudent(state, studentId)
    .filter((t) => t.type === 'Turnaus' && t.loggedBy === 'player')
    .sort((a, b) => b.date.localeCompare(a.date));
}
// The tournament (if any) a given day falls inside, for defaulting MatchNoteModal's picker
// when a match is logged from within a tournament's date range.
function koutsiTournamentOnDateForStudent(state, studentId, dateStr) {
  return koutsiTournamentsForStudent(state, studentId)
    .find((t) => dateStr >= t.date && dateStr <= (t.endDate || t.date)) || null;
}
// A tournament entry has no name field of its own — a player types the tournament's name
// into its free-text `notes` (see SELF_TRAINING_NOTE_HINTS.Turnaus in koutsi-pelaaja-app.jsx),
// so that's the label wherever a tournament needs a one-line name; falls back to its date(s)
// for the (rare) tournament logged with no description.
function koutsiTournamentLabel(t) {
  if (!t) return '';
  if (t.notes && t.notes.trim()) return t.notes.trim();
  const range = t.endDate && t.endDate !== t.date ? `${koutsiFmtShortDate(t.date)}–${koutsiFmtShortDate(t.endDate)}` : koutsiFmtShortDate(t.date);
  return `Turnaus ${range}`;
}
...
function koutsiClubEventsOnDate(state, dateStr) {
  return (state.clubEvents || []).filter((e) => e.date <= dateStr && (e.endDate || e.date) >= dateStr);
}
function koutsiUpcomingClubEvents(state) {
  const today = koutsiTodayStr();
  return (state.clubEvents || []).filter((e) => (e.endDate || e.date) >= today).slice().sort((a, b) => a.date.localeCompare(b.date));
}
...
const KOUTSI_TAG_LABELS = { syotto: 'Syöttö', liikkuminen: 'Liikkuminen', pistepeli: 'Pistepeli', verkkopeli: 'Verkkopeli', tekniikka: 'Tekniikka', lammittely: 'Lämmittely', fysiikka: 'Fysiikka', drilli: 'Drilli' };
const KOUTSI_TAGS = ['syotto', 'liikkuminen', 'pistepeli', 'verkkopeli', 'tekniikka', 'lammittely', 'fysiikka', 'drilli'];
...
function koutsiStudentAge(studentRow, profileRow) {
  const hasAccount = Boolean(profileRow);
  const value = hasAccount
    ? (profileRow.age || '')
    : (studentRow.age == null ? '' : String(studentRow.age));
  const exact = /^\d{1,3}$/.test(value) ? Number(value) : null;
  return {
    age: exact,
    ageRange: null,
    ageValue: exact == null ? '' : String(exact),
    ageLabel: exact == null ? '' : `${exact} v`,
  };
}
...
function koutsiMapGroup(g, memberIds, themeRows, slotRows) {
  ...
  // Extra weekly training times beyond the group's own primary weekday/time below.
  const slots = (slotRows || [])
    .filter((s) => s.group_id === g.id)
    .map((s) => ({ id: s.id, day: s.weekday, time: s.time ? s.time.slice(0, 5) : '', durationMinutes: s.duration_minutes || 60 }));
  return {
...
function koutsiMapTraining(t, absenceRows) {
  return {
    ...
    loggedBy: t.logged_by || 'coach', notes: t.notes || '', endDate: t.end_date || null,
...
function koutsiMapExercise(e) {
  const hasVideo = Boolean(e.video_storage_path || e.video_external_url);
  return {
    id: e.id, coachId: e.coach_id, name: e.name, goal: e.goal, players: e.players_label,
    playerCount: e.player_count, duration: e.duration, level: e.level, tags: e.tags || [],
    video: hasVideo ? {
      title: e.video_title || e.name,
      storagePath: e.video_storage_path || null,
      externalUrl: e.video_external_url || null,
      mimeType: e.video_mime_type || null,
      sizeBytes: e.video_size_bytes || null,
    } : null,
  };
}
function koutsiMapCoachEvent(e) { return { id: e.id, date: e.date, endDate: e.end_date || null, title: e.title, kind: e.kind }; }
...
function koutsiMapStudent(studentRow, profileRow, diaryRows, homeworkRows, videoRows, moodRows, matchNoteRows, historyRows) {
  const age = koutsiStudentAge(studentRow, profileRow);
  ...
  return {
    ...
    welcomeSeenAt: studentRow.welcome_seen_at || null,
    ...
    matchNotes: (matchNoteRows || []).filter((n) => n.student_id === studentRow.id).map((n) => ({
      id: n.id, at: n.created_at, opponentName: n.opponent_name, date: n.date, note: n.note || '',
      durationMinutes: n.duration_minutes || null, result: n.result || null, format: n.format || null, score: n.score || '',
      partnerName: n.partner_name || '', opponent2Name: n.opponent2_name || '', tournamentId: n.tournament_id || null,
    })),
...
function koutsiMapCoach(coachRow, profileRow) {
  return {
    ...
    welcomeSeenAt: coachRow.welcome_seen_at || null,
  };
}
...
async function koutsiLoadCoachState(coachId) {
  const [{ data: coachRow, error: coachErr }, { data: profileRow }] = await Promise.all([
    koutsiSupabase.from('koutsi_coaches').select('id, tagline, bio, experience, specialties, welcome_seen_at').eq('id', coachId).single(),
  ...
  const [
    ...
    studentIds.length ? koutsiSupabase.from('profiles').select('id, name, avatar_url, age').in('id', studentIds) : Promise.resolve({ data: [] }),
  ...
  const [{ data: memberRows }, { data: themeRows }, { data: slotRows }] = groupIds.length
    ? await Promise.all([
      ...
      koutsiSupabase.from('koutsi_group_slots').select('*').in('group_id', groupIds),
    ])
    : [{ data: [] }, { data: [] }, { data: [] }];
  ...
  const groups = (groupRows || []).map((g) => koutsiMapGroup(g, (memberRows || []).filter((m) => m.group_id === g.id).map((m) => m.student_id), themeRows || [], slotRows || []));
...
async function koutsiLoadStudentState(studentId) {
  const [{ data: studentRow, error: studentErr }, { data: profileRow }] = await Promise.all([
    ...
    koutsiSupabase.from('profiles').select('id, name, avatar_url, avatar_color, age').eq('id', studentId).maybeSingle(),
  ]);
  ...
  const [
    ...
    { data: groupRows }, { data: rosterRows, error: rosterErr }, { data: themeRows }, { data: slotRows },
    ...
    groupIds.length ? koutsiSupabase.from('koutsi_group_slots').select('*').in('group_id', groupIds) : Promise.resolve({ data: [] }),
  ...
  const groups = (groupRows || []).map((g) => koutsiMapGroup(g, (rosterRows || []).filter((m) => m.group_id === g.id).map((m) => m.student_id), themeRows || [], slotRows || []));
  const trainings = trainingRows.map((t) => koutsiMapTraining(t, absenceRows || []));
...
const KOUTSI_ERROR_PATTERNS = [
  ...
  [/liity ensin valmentajan sinulle lähettämällä henkilökohtaisella linkillä/i, 'Alaikäisen tili aktivoidaan valmentajan lähettämästä henkilökohtaisesta liittymislinkistä. Pyydä valmentajalta uusi linkki.'],
  [/alaikäinen tietää Koutsin käytöstä/i, 'Valmentajan pitää ensin vahvistaa, että alaikäinen tietää Koutsin käytöstä.'],
  [/alle 13-vuotiaan.*huoltajan hyväksynnän/i, 'Alle 13-vuotiaan tilin aktivointi vaatii huoltajan hyväksynnän, jonka valmentaja vahvistaa kutsussa.'],
  [/aikuisten beta-pilotissa|rajattu vähintään 18-vuotiaille/i, 'Tämä vanha pilottirajaus ei ole enää käytössä. Päivitä sivu ja yritä uudelleen.'],
...
// A coach picks day + time on a dial/keyboard that can land anywhere; the calendar (and
// the DB constraint backing it) only accepts quarter-hour marks, so every entry point
// rounds through this before it's stored.
function koutsiRoundTimeToQuarterHour(hhmm) {
  if (!hhmm) return hhmm;
  const [h, m] = hhmm.split(':').map(Number);
  if (!Number.isFinite(h) || !Number.isFinite(m)) return hhmm;
  const wrapped = (((Math.round((h * 60 + m) / 15) * 15) % 1440) + 1440) % 1440;
  return `${koutsiPad(Math.floor(wrapped / 60))}:${koutsiPad(wrapped % 60)}`;
}
// A coach can type any minute count into a duration field; the DB constraint (and the
// calendar's quarter-hour grid) only accepts 15-480 in steps of 15, so every entry point
// rounds through this before it's stored — same idea as koutsiRoundTimeToQuarterHour.
function koutsiRoundToQuarterHourMinutes(minutes) {
  const n = Number(minutes);
  if (!Number.isFinite(n) || n <= 0) return 60;
  const rounded = Math.round(n / 15) * 15;
  return Math.min(480, Math.max(15, rounded));
}
function koutsiAddMinutesToTime(hhmm, minutes) {
  if (!hhmm || !minutes) return hhmm;
  const [h, m] = hhmm.split(':').map(Number);
  const wrapped = (((h * 60 + m + minutes) % 1440) + 1440) % 1440;
  return `${koutsiPad(Math.floor(wrapped / 60))}:${koutsiPad(wrapped % 60)}`;
}
function koutsiFmtDuration(minutes) {
  if (!minutes) return '';
...
// "14:00" alone, or "14:00–15:30" once a duration is known.
function koutsiTimeRangeLabel(hhmm, minutes) {
  if (!hhmm) return '';
  return minutes ? `${hhmm}–${koutsiAddMinutesToTime(hhmm, minutes)}` : hhmm;
}
// First date on/after `fromDate` that falls on the group's weekday.
function koutsiNextWeekday(fromDate, dayCode) {
  const targetIdx = KOUTSI_WEEKDAY_TO_INDEX[(dayCode || '').toLowerCase()];
  if (targetIdx == null) return null;
  const diff = (targetIdx - koutsiDateFromStr(fromDate).getDay() + 7) % 7;
  return koutsiAddDays(fromDate, diff);
}
// How many weeks of sessions a group's weekly slot materializes by default, and the most
// a coach can ask for at once — creating a group (or an extra slot) floods the calendar
// with one row per week, so this stays well under a full year unless raised explicitly.
const KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS = 12;
const KOUTSI_MAX_GROUP_TRAINING_WEEKS = 52;
function koutsiClampGroupTrainingWeeks(weeksAhead) {
  const value = Math.round(Number(weeksAhead));
  if (!Number.isFinite(value)) return KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS;
  return Math.min(KOUTSI_MAX_GROUP_TRAINING_WEEKS, Math.max(1, value));
}
// Materializes a group's weekly slot into koutsi_trainings so it shows up on the
// calendar the moment the group is created, the same way a manually "toistuva" single
// training does. `weeksAhead` defaults to KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS and is
// clamped to KOUTSI_MAX_GROUP_TRAINING_WEEKS, same safety cap as koutsiWeeklyDates.
async function koutsiGenerateGroupTrainings(groupId, coachId, day, time, durationMinutes, fromDate, weeksAhead) {
  const first = koutsiNextWeekday(fromDate || koutsiTodayStr(), day);
  if (!first || !time) return { count: 0, seriesId: null };
  const span = (koutsiClampGroupTrainingWeeks(weeksAhead ?? KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS) - 1) * 7;
  const dates = koutsiWeeklyDates(first, koutsiAddDays(first, span));
  const seriesId = koutsiRandomUuid();
  const rows = dates.map((d) => ({
    coach_id: coachId, group_id: groupId, date: d, time, type: 'Ryhmätreeni',
    duration_minutes: durationMinutes || null, series_id: seriesId,
  }));
  const { error } = await koutsiSupabase.from('koutsi_trainings').insert(rows);
  if (error) throw error;
  return { count: rows.length, seriesId };
}
// Extra weekly training times for a group beyond its primary weekday/time (koutsi_groups
// itself) — e.g. a group that trains both Tuesday and Thursday. Each slot materializes
// its own weekly koutsi_trainings series, so it shows up everywhere the primary slot's
// sessions already do, with no separate rendering path.
async function koutsiAddGroupSlot({ groupId, coachId, day, time, durationMinutes, weeksAhead }) {
  const roundedTime = koutsiRoundTimeToQuarterHour(time);
  const duration = koutsiRoundToQuarterHourMinutes(durationMinutes || 60);
  const { data, error } = await koutsiSupabase.from('koutsi_group_slots')
    .insert({ group_id: groupId, coach_id: coachId, weekday: day, time: roundedTime, duration_minutes: duration })
    .select('id').single();
  if (error) throw error;
  const { seriesId } = await koutsiGenerateGroupTrainings(groupId, coachId, day, roundedTime, duration, null, weeksAhead);
  if (seriesId) {
    const { error: linkErr } = await koutsiSupabase.from('koutsi_group_slots').update({ series_id: seriesId }).eq('id', data.id);
    if (linkErr) throw linkErr;
  }
  return data.id;
}
// Deletes the slot and its not-yet-happened sessions (past sessions, with any attendance
// already taken, are left alone — same rule koutsiUpdateGroup follows for the primary slot).
async function koutsiDeleteGroupSlot(slotId) {
  const { data: slot, error: readErr } = await koutsiSupabase.from('koutsi_group_slots').select('series_id').eq('id', slotId).maybeSingle();
  if (readErr) throw readErr;
  if (slot?.series_id) {
    const { error: delErr } = await koutsiSupabase.from('koutsi_trainings')
      .delete().eq('series_id', slot.series_id).gte('date', koutsiTodayStr());
    if (delErr) throw delErr;
  }
  const { error } = await koutsiSupabase.from('koutsi_group_slots').delete().eq('id', slotId);
  if (error) throw error;
}
...
// Player-logged individual sessions — self-directed practice, physical training or
// another sport, alongside whatever the coach schedules. `type` doubles as the category
// label (see koutsi-pelaaja-app.jsx's SELF_TRAINING_TYPES) so it renders everywhere a
// training's type already does, with no extra plumbing.
async function koutsiAddSelfTraining({ studentId, coachId, date, endDate, time, type, durationMinutes, notes }) {
  const { error } = await koutsiSupabase.from('koutsi_trainings').insert({
    coach_id: coachId, student_id: studentId, group_id: null, date, end_date: endDate || null, time, type,
    duration_minutes: durationMinutes || null, notes: notes || null, logged_by: 'player',
  });
  if (error) throw error;
}
async function koutsiDeleteSelfTraining(trainingId) {
  const { error } = await koutsiSupabase.from('koutsi_trainings').delete().eq('id', trainingId);
  if (error) throw error;
}
...
async function koutsiUploadVideoFile(uploaderId, shareId, file, onProgress) {
  ...
  // Storage host. The static Koutsi app never proxies or buffers the video. TUS can
...
async function koutsiShareVideo({ shareId, title, date, tags, studentIds, addedById, file, externalUrl, onProgress }) {
  ...
  // A retry after a lost response (or an accidental double-submit) must not insert a
  // second copy of rows this share already has — there's no DB constraint stopping that,
  // so check what's already there first, the same way koutsi_set_video_recipients does.
  const { data: existingRows, error: existingErr } = await koutsiSupabase.from('koutsi_videos')
    .select('student_id').eq('share_id', resolvedShareId);
  if (existingErr) throw existingErr;
  const alreadyShared = new Set((existingRows || []).map((r) => r.student_id));
  const pendingStudentIds = studentIds.filter((id) => !alreadyShared.has(id));
...
// A coach can only edit name/age/level for a player who hasn't claimed an account yet
// (isPlaceholder): once claimed, the name and age become player-owned (koutsiSaveDisplayName /
// koutsiSaveStudentProfile) and a DB trigger overwrites koutsi_students.age from the profile
// on every update anyway, so a coach write there would silently do nothing.
async function koutsiUpdatePlaceholderStudent(studentId, { name, age, level }) {
  if (age != null && (!Number.isInteger(age) || age < 1 || age >= 120)) {
    throw new Error('Pelaajan iän pitää olla 1–119 vuotta.');
  }
  const patch = {};
...
async function koutsiEndCoaching(coachId, studentId) {
  ...
  const { data: groups, error: groupsErr } = await koutsiSupabase.from('koutsi_groups').select('id').eq('coach_id', coachId);
  if (groupsErr) throw groupsErr;
  const groupIds = (groups || []).map((g) => g.id);
  if (groupIds.length) {
    const { error: membersErr } = await koutsiSupabase.from('koutsi_group_members')
      .update({ ended_at: new Date().toISOString() })
      .eq('student_id', studentId).in('group_id', groupIds).is('ended_at', null);
    if (membersErr) throw membersErr;
  }
}
// Permanently erases a placeholder player and everything tied to them — every table with
// a student_id foreign key cascades on delete, so removing the koutsi_students row alone
// clears it from groups, trainings, diary, homework, videos, moods and attendance too.
// Restricted server-side to still-unclaimed placeholders: a player with their own login
// can only ever be unlinked (koutsiEndCoaching), never erased by their coach.
async function koutsiDeletePlayer(coachId, studentId) {
  const { error } = await koutsiSupabase.rpc('koutsi_delete_placeholder_student', {
    coach_id_input: coachId, student_id_input: studentId,
  });
  if (error) throw error;
}
// Collapses an accidental duplicate into the real profile: every group, training, diary
// entry, homework, video, mood, match note and history row the duplicate had moves onto
// `keepStudentId`, then the duplicate row is gone. Only works one direction — the student
// being removed must still be an unclaimed placeholder, enforced server-side.
async function koutsiMergeStudents(coachId, keepStudentId, removeStudentId) {
  const { data, error } = await koutsiSupabase.rpc('koutsi_merge_students', {
    coach_id_input: coachId || null,
    keep_student_id: keepStudentId,
    remove_student_id: removeStudentId,
  });
  if (error) throw error;
  return data;
}
...
// ── groups ───────────────────────────────────────────────────
async function koutsiCreateGroup({ coachId, name, level, day, time, durationMinutes, memberIds, weeksAhead }) {
  const duration = durationMinutes || 60;
  const { data, error } = await koutsiSupabase.from('koutsi_groups')
    .insert({ coach_id: coachId, name, level, weekday: day, time, duration_minutes: duration }).select('id').single();
  if (error) throw error;
  ...
  await koutsiGenerateGroupTrainings(data.id, coachId, day, time, duration, null, weeksAhead);
  return data.id;
}
// Renaming a group or changing its level doesn't touch the calendar. Moving its weekly
// slot does: future sessions (not-yet-happened, so no attendance to lose) are dropped and
// regenerated for the new day/time/duration. Past sessions are left alone.
async function koutsiUpdateGroup(groupId, { name, level, day, time, durationMinutes, weeksAhead }) {
  const duration = durationMinutes || 60;
  const { data: existing, error: exErr } = await koutsiSupabase.from('koutsi_groups')
    .select('coach_id, weekday, time, duration_minutes').eq('id', groupId).single();
  if (exErr) throw exErr;
  const { error } = await koutsiSupabase.from('koutsi_groups')
    .update({ name, level, weekday: day, time, duration_minutes: duration }).eq('id', groupId);
  if (error) throw error;
  const scheduleChanged = existing.weekday !== day
    || (existing.time || '').slice(0, 5) !== (time || '').slice(0, 5)
    || (existing.duration_minutes || 60) !== duration;
  if (scheduleChanged) {
    const today = koutsiTodayStr();
    // Extra weekly slots (koutsiAddGroupSlot) generate their own series under this same
    // group_id — excluding their series here so moving the primary time doesn't also wipe
    // sessions that belong to a different weekly slot entirely.
    const { data: slots, error: slotErr } = await koutsiSupabase.from('koutsi_group_slots')
      .select('series_id').eq('group_id', groupId).not('series_id', 'is', null);
    if (slotErr) throw slotErr;
    const slotSeriesIds = (slots || []).map((s) => s.series_id).filter(Boolean);
    let deleteQuery = koutsiSupabase.from('koutsi_trainings').delete().eq('group_id', groupId).gte('date', today);
    if (slotSeriesIds.length) deleteQuery = deleteQuery.not('series_id', 'in', `(${slotSeriesIds.join(',')})`);
    const { error: delErr } = await deleteQuery;
    if (delErr) throw delErr;
    await koutsiGenerateGroupTrainings(groupId, existing.coach_id, day, time, duration, today, weeksAhead);
  }
}
...
// A shared workbook is a coach-level review submission, not a direct theme import. The
// metadata insert triggers both the admin queue and its retryable email notification.
async function koutsiUploadSharedAnnualPlan(coachId, file) {
  if (!file || !/\.xlsx$/i.test(file.name || '')) {
    throw new Error('Valitse .xlsx-muotoinen Excel-tiedosto.');
  }
  if (file.size < 1 || file.size > KOUTSI_PLAN_MAX_BYTES) {
    throw new Error(`Tiedosto on liian suuri (${Math.round(file.size / 1048576)} Mt). Enimmäiskoko on 20 Mt.`);
  }
  const path = `shared/${coachId}/${koutsiRandomUuid()}.xlsx`;
  const { error: uploadError } = await koutsiSupabase.storage.from('koutsi-plans').upload(path, file, {
    contentType: file.type || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    upsert: false,
  });
  if (uploadError) throw uploadError;

  const { data, error } = await koutsiSupabase.from('koutsi_annual_plan_submissions').insert({
    coach_id: coachId,
    filename: file.name.slice(0, 240),
    storage_path: path,
    size_bytes: file.size,
  }).select('id, coach_id, filename, storage_path, size_bytes, status, uploaded_at, handled_at').single();
  if (!error) return data;

  const { error: cleanupError } = await koutsiSupabase.storage.from('koutsi-plans').remove([path]);
  if (cleanupError) throw new Error(`${error.message}; tiedoston siivous epäonnistui: ${cleanupError.message}`);
  throw error;
}

async function koutsiAnnualPlanSubmissions(coachId = null, pendingOnly = false) {
  let query = koutsiSupabase.from('koutsi_annual_plan_submissions')
    .select('id, coach_id, filename, storage_path, size_bytes, status, uploaded_at, handled_at')
  ...
  if (error) throw error;
  return (data || []).map((row) => ({
    id: row.id, coachId: row.coach_id, filename: row.filename,
    storagePath: row.storage_path, sizeBytes: Number(row.size_bytes || 0),
    status: row.status, uploadedAt: row.uploaded_at, handledAt: row.handled_at,
  }));
}

async function koutsiAdminHandleAnnualPlanSubmission(submissionId) {
  const { error } = await koutsiSupabase.rpc('koutsi_admin_handle_annual_plan_submission', {
...
async function koutsiDeleteExercise(exerciseId) {
  const { data: exercise, error: readErr } = await koutsiSupabase.from('koutsi_exercises').select('*').eq('id', exerciseId).maybeSingle();
  if (readErr) throw readErr;
  if (!exercise) return;

  const { error } = await koutsiSupabase.from('koutsi_exercises').delete().eq('id', exerciseId);
  if (error) throw error;
  const path = exercise.video_storage_path || null;
  if (!path) return;

  const { error: storageErr } = await koutsiSupabase.storage.from('koutsi-videos').remove([path]);
  if (!storageErr) return;
  const { error: rollbackErr } = await koutsiSupabase.from('koutsi_exercises').insert(exercise);
  if (rollbackErr) {
    throw new Error(`${storageErr.message}; harjoitteen palautus epäonnistui: ${rollbackErr.message}`);
  }
  throw storageErr;
}
async function koutsiSetExerciseVideo({ exerciseId, coachId, title, file, externalUrl, onProgress }) {
  const { data: exercise, error: readErr } = await koutsiSupabase.from('koutsi_exercises').select('*').eq('id', exerciseId).single();
  if (readErr) throw readErr;

  let uploaded = null;
  if (file) {
    uploaded = await koutsiUploadVideoFile(coachId, `exercise-${koutsiRandomUuid()}`, file, onProgress);
  }
  const next = {
    video_title: title,
    video_storage_path: uploaded ? uploaded.path : null,
    video_external_url: externalUrl || null,
    video_mime_type: uploaded ? uploaded.mimeType : null,
    video_size_bytes: uploaded ? uploaded.sizeBytes : null,
  };
  const previous = {
    video_title: exercise.video_title || null,
    video_storage_path: exercise.video_storage_path || null,
    video_external_url: exercise.video_external_url || null,
    video_mime_type: exercise.video_mime_type || null,
    video_size_bytes: exercise.video_size_bytes || null,
  };

  const { error } = await koutsiSupabase.from('koutsi_exercises').update(next).eq('id', exerciseId);
  if (error) {
    if (uploaded) await koutsiSupabase.storage.from('koutsi-videos').remove([uploaded.path]);
    throw error;
  }

  const oldPath = previous.video_storage_path;
  if (!oldPath || oldPath === next.video_storage_path) return;
  const { error: storageErr } = await koutsiSupabase.storage.from('koutsi-videos').remove([oldPath]);
  if (!storageErr) return;

  const { error: rollbackErr } = await koutsiSupabase.from('koutsi_exercises').update(previous).eq('id', exerciseId);
  let cleanupErr = null;
  if (uploaded) {
    const cleanup = await koutsiSupabase.storage.from('koutsi-videos').remove([uploaded.path]);
    cleanupErr = cleanup.error || null;
  }
  if (rollbackErr || cleanupErr) {
    throw new Error(`${storageErr.message}; videon vaihdon palautus epäonnistui: ${(rollbackErr || cleanupErr).message}`);
  }
  throw storageErr;
}
async function koutsiRemoveExerciseVideo(exerciseId) {
  const { data: exercise, error: readErr } = await koutsiSupabase.from('koutsi_exercises').select('*').eq('id', exerciseId).single();
  if (readErr) throw readErr;
  if (!exercise.video_storage_path && !exercise.video_external_url) return;
  ...
  const empty = {
    video_title: null, video_storage_path: null, video_external_url: null,
    video_mime_type: null, video_size_bytes: null,
  };
  const { error } = await koutsiSupabase.from('koutsi_exercises').update(empty).eq('id', exerciseId);
  if (error) throw error;
  if (!previous.video_storage_path) return;

  const { error: storageErr } = await koutsiSupabase.storage.from('koutsi-videos').remove([previous.video_storage_path]);
  if (!storageErr) return;
  const { error: rollbackErr } = await koutsiSupabase.from('koutsi_exercises').update(previous).eq('id', exerciseId);
  if (rollbackErr) {
    throw new Error(`${storageErr.message}; harjoitevideon palautus epäonnistui: ${rollbackErr.message}`);
  }
  throw storageErr;
}
...
// Replaces the coach's permanent code with a fresh one. The old code stops resolving to
// anything the instant this returns — koutsi_coach_for_code/redeem_koutsi_invite_code only
// ever look a code up by its current value, so there is nothing else to invalidate.
async function koutsiRotateJoinCode() {
  const { data, error } = await koutsiSupabase.rpc('koutsi_rotate_join_code');
  if (error) throw error;
  return data;
}
...
async function koutsiCreatePlayer(name, age, level, coachId = null) {
  if (age != null && (!Number.isInteger(age) || age < 1 || age >= 120)) {
    throw new Error('Jos annat pelaajan iän, sen pitää olla 1–119 vuotta.');
  }
  const { data, error } = await koutsiSupabase.rpc('koutsi_create_player_v2', {
...
async function koutsiBulkSetup({ coachId = null, groups = [], players = [], themes = [], weeksAhead } = {}) {
  if (players.some((player) => player.age != null && (!Number.isInteger(player.age) || player.age < 1 || player.age >= 120))) {
    throw new Error('Jos annat pelaajan iän, sen pitää olla 1–119 vuotta.');
  }
  const { data, error } = await koutsiSupabase.rpc('koutsi_bulk_setup_v2', {
    payload: { groups, players, themes, weeks_ahead: koutsiClampGroupTrainingWeeks(weeksAhead ?? KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS) },
    coach_id_input: coachId || null,
  });
  if (error) throw error;
  return data;
...
// Called before redeeming a shared (non-personal) code: if it matches exactly one of the
// coach's not-yet-claimed named players, the join screen can offer "is this you?" instead
// of always creating a second, empty profile for someone the coach already added by name.
// Returns { id, name } or null — never a list, so a person holding the shared code still
// can't use this to browse the coach's roster.
async function koutsiMatchUnclaimedPlayer(code) {
  const { data, error } = await koutsiSupabase.rpc('koutsi_match_unclaimed_player', { code_input: code });
  if (error) throw error;
  return data || null;
}
// Fallback for when the automatic match above found nothing: the join screen only calls
// this after the player explicitly says "I think I'm already on the list", so the names it
// returns (the coach's — or that specific group's, for a group-scoped code — not-yet-
// claimed players) aren't shown to someone who never asked for them.
async function koutsiUnclaimedPlayersForCode(code) {
  const { data, error } = await koutsiSupabase.rpc('koutsi_unclaimed_players_for_code', { code_input: code });
  if (error) throw error;
  return data || [];
}
// This placeholder's own persistent, genuinely unique join code — generated on first call,
// including for a placeholder created before this existed. Unlike the coach's shared
// permanent code, this one resolves to exactly this player and no other.
async function koutsiPlaceholderJoinCode(studentId) {
  const { data, error } = await koutsiSupabase.rpc('koutsi_placeholder_join_code', { student_id_input: studentId });
  if (error) throw error;
  return data;
}
...
function koutsiInviteLink(code, studentId = null) {
  const params = new URLSearchParams({ koodi: code });
  if (studentId) params.set('oppilas', studentId);
  return `https://koutsi.krossi.app/pelaaja?${params.toString()}`;
}
function koutsiInviteMessage(code, coachName, groupName, studentId = null) {
  const who = coachName ? `${coachName} täällä` : 'Moi';
  const where = groupName ? ` ryhmään ${groupName}` : '';
  return `${who}! Otetaan Krossi Koutsi käyttöön — sieltä näet treenit, kotiläksyt ja palautteet. Ikää ei tarvitse ilmoittaa, eikä Koutsiin kirjata terveystietoja.\n\nLiity${where} henkilökohtaisesta linkistä: ${koutsiInviteLink(code, studentId)}${studentId ? '' : `\n\n(Jos linkki ei aukea, mene osoitteeseen koutsi.krossi.app/pelaaja ja syötä koodi ${code}.)`}`;
}
...
async function koutsiAddMatchNote(studentId, { opponentName, date, note, durationMinutes, result, format, score, partnerName, opponent2Name, tournamentId }) {
  const { error } = await koutsiSupabase.from('koutsi_match_notes').insert({
    student_id: studentId, opponent_name: opponentName, date, note: note || null,
    duration_minutes: durationMinutes || null, result: result || null, format: format || null, score: score || null,
    partner_name: format === 'nelinpeli' ? (partnerName || null) : null,
    opponent2_name: format === 'nelinpeli' ? (opponent2Name || null) : null,
    tournament_id: tournamentId || null,
  });
  if (error) throw error;
}
async function koutsiUpdateMatchNote(noteId, { opponentName, date, note, durationMinutes, result, format, score, partnerName, opponent2Name, tournamentId }) {
  const { error } = await koutsiSupabase.from('koutsi_match_notes')
    .update({
      opponent_name: opponentName, date, note: note || null,
      duration_minutes: durationMinutes || null, result: result || null, format: format || null, score: score || null,
      partner_name: format === 'nelinpeli' ? (partnerName || null) : null,
      opponent2_name: format === 'nelinpeli' ? (opponent2Name || null) : null,
      tournament_id: tournamentId || null,
...
// Pilotissa pelaaja voi päivittää vain ikänsä. Jaettu profiili ja oppilasrivi saavat
// tarkan iän vain silloin, kun pelaaja on itse antanut sen.
// Taustatietokenttä on lukittu, koska se
// pyysi aiemmin suoraan erityisiin henkilötietoryhmiin kuuluvia terveystietoja.
async function koutsiSaveStudentProfile(studentId, { age, profileAge }) {
  if (profileAge !== undefined) {
    const { error: profileError } = await koutsiSupabase.from('profiles').update({ age: profileAge }).eq('id', studentId);
...
async function koutsiSubmitFeedback(studentId, { category, message }) {
  const { error } = await koutsiSupabase.from('koutsi_feedback')
    .insert({ student_id: studentId, category, message });
  if (error) throw error;
}
async function koutsiMarkWelcomeSeen(studentId) {
  const { error } = await koutsiSupabase.from('koutsi_students')
    .update({ welcome_seen_at: new Date().toISOString() }).eq('id', studentId);
  if (error) throw error;
}
async function koutsiMarkCoachWelcomeSeen(coachId) {
  const { error } = await koutsiSupabase.from('koutsi_coaches')
    .update({ welcome_seen_at: new Date().toISOString() }).eq('id', coachId);
  if (error) throw error;
}
...
async function koutsiAddClubEvent({ coachId, date, endDate, title, kind }) {
  const { error } = await koutsiSupabase.from('koutsi_coach_events')
    .insert({ coach_id: coachId, date, end_date: endDate || null, title, kind: kind || 'seura' });
  if (error) throw error;
}
async function koutsiUpdateClubEvent(eventId, { date, endDate, title, kind }) {
  const { error } = await koutsiSupabase.from('koutsi_coach_events')
    .update({ date, end_date: endDate || null, title, kind: kind || 'seura' }).eq('id', eventId);
  if (error) throw error;
}
...
function koutsiAttendanceSummary(state, studentId, sinceDate) {
  const today = koutsiTodayStr();
  // Player-logged practice has no coach-run session to be present or absent from.
  const all = koutsiTrainingsForStudent(state, studentId)
    .filter((t) => t.loggedBy !== 'player' && t.date <= today && (!sinceDate || t.date >= sinceDate));
...
// ── monthly summary (Strava-like overview for a player's own month) ──────────
// Past-only, like koutsiAttendanceSummary: a month still in progress should read as
// "what you've actually done", not inflated by sessions that have not happened yet.
// Buckets are drawn from fields that already exist on a training/match note — no new
// category column needed. Reused by the coach side to spot an inactive player (Osa 5).
const KOUTSI_MONTHLY_CATEGORIES = [
  { key: 'ryhmatreeni', label: 'Ryhmätreenit' },
  { key: 'valmennus', label: 'Muut valmennukset' },
  { key: 'omatoiminen', label: 'Omatoimiset harjoitukset' },
  { key: 'fysiikka', label: 'Fysiikka' },
  { key: 'muu', label: 'Muu liikunta' },
  { key: 'turnaus', label: 'Turnaukset' },
  { key: 'ottelu', label: 'Ottelut' },
];
function koutsiMonthlySummary(state, studentId, year, month) {
  const monthPrefix = `${year}-${koutsiPad(month + 1)}`;
  const today = koutsiTodayStr();
  const buckets = {};
  KOUTSI_MONTHLY_CATEGORIES.forEach((c) => { buckets[c.key] = { key: c.key, label: c.label, count: 0, minutes: 0 }; });

  const trainings = koutsiTrainingsForStudent(state, studentId)
    .filter((t) => t.date.startsWith(monthPrefix) && t.date <= today);
  for (const t of trainings) {
    const key = t.groupId != null ? 'ryhmatreeni'
      : t.loggedBy === 'player' ? (t.type === 'Fysiikka' ? 'fysiikka' : t.type === 'Muu liikunta' ? 'muu' : t.type === 'Turnaus' ? 'turnaus' : 'omatoiminen')
      : 'valmennus';
    buckets[key].count += 1;
    buckets[key].minutes += t.durationMinutes || 0;
  }

  const student = koutsiStudentById(state, studentId);
  const matches = (student?.matchNotes || []).filter((n) => n.date.startsWith(monthPrefix) && n.date <= today);
  buckets.ottelu.count = matches.length;
  buckets.ottelu.minutes = matches.reduce((sum, n) => sum + (n.durationMinutes || 0), 0);
  const wins = matches.filter((n) => n.result === 'voitto').length;
  const losses = matches.filter((n) => n.result === 'tappio').length;

  const categories = KOUTSI_MONTHLY_CATEGORIES.map((c) => buckets[c.key]);
  const totalSessions = trainings.length + matches.length;
  const totalMinutes = categories.reduce((sum, c) => sum + c.minutes, 0);
  return { categories, totalSessions, totalMinutes, matchWins: wins, matchLosses: losses };
}
...
async function koutsiAdminUsers() {
  const [{ data, error }, submissions] = await Promise.all([
    koutsiSupabase.rpc('koutsi_admin_users'),
    koutsiAnnualPlanSubmissions(null, true),
  ]);
  if (error) throw error;
  const pendingSharedByCoach = submissions.reduce((counts, submission) => {
    counts.set(submission.coachId, (counts.get(submission.coachId) || 0) + 1);
    return counts;
  }, new Map());
  return (data || []).map((r) => ({
    id: r.user_id,
    ...
    lastAppOpenAt: r.last_app_open_at,
    ...
    pendingPlans: (r.pending_plans || 0) + (pendingSharedByCoach.get(r.user_id) || 0),
    pendingStudents: r.pending_students || [],
  }));
}
async function koutsiAdminFeedback() {
  const { data, error } = await koutsiSupabase.rpc('koutsi_admin_feedback');
  if (error) throw error;
  return (data || []).map((r) => ({
    id: r.id,
    senderId: r.sender_id,
    senderName: r.sender_name || 'Nimetön',
    email: r.email || '',
    isCoach: Boolean(r.is_coach),
    isPlayer: Boolean(r.is_player),
    category: r.category,
...
async function koutsiAdminDeleteFeedback(feedbackId) {
  const { error } = await koutsiSupabase.rpc('koutsi_admin_delete_feedback', { feedback_id_input: feedbackId });
  if (error) throw error;
}
...
Object.assign(window, {
  ...
  koutsiStudentById, koutsiGroupById, koutsiGroupsForStudent, koutsiCoachById, koutsiTrainingParty,
  koutsiThemeForDate,
  koutsiTrainingsForStudent, koutsiUpcomingTrainingsForStudent, koutsiTrainingsForGroup,
  koutsiTournamentsForStudent, koutsiTournamentOnDateForStudent, koutsiTournamentLabel,
  ...
  koutsiAttendanceSummary, koutsiMonthlySummary,
  koutsiIsAdmin, koutsiAdminActAs, koutsiPendingAnnualPlans, koutsiPublishAnnualPlan,
  koutsiAdminCoaches, koutsiAdminUsers, koutsiAdminFeedback, koutsiAdminDeleteFeedback, koutsiAdminDeleteUser,
  koutsiAdminGroups, koutsiAdminBulkInviteCodes,
  koutsiAdminUploadAnnualPlan, koutsiParseNameList,
  koutsiUploadSharedAnnualPlan, koutsiAnnualPlanSubmissions, koutsiAdminHandleAnnualPlanSubmission,
  ...
  koutsiAddSelfTraining, koutsiDeleteSelfTraining,
  koutsiRoundTimeToQuarterHour, koutsiRoundToQuarterHourMinutes, koutsiAddMinutesToTime, koutsiFmtDuration, koutsiTimeRangeLabel,
  koutsiGenerateGroupTrainings, koutsiNextWeekday,
  koutsiAddGroupSlot, koutsiDeleteGroupSlot,
  koutsiClampGroupTrainingWeeks, KOUTSI_DEFAULT_GROUP_TRAINING_WEEKS, KOUTSI_MAX_GROUP_TRAINING_WEEKS,
  koutsiShareVideo, koutsiSetVideoRecipients, koutsiVideoUrl, koutsiDeleteVideo,
  koutsiSetAttendance, koutsiCycleAbsence,
  koutsiSetStudentLevel, koutsiUpdatePlaceholderStudent, koutsiEndCoaching, koutsiMergeStudents, koutsiDeletePlayer,
  ...
  koutsiAddExercise, koutsiUpdateExercise, koutsiDeleteExercise,
  koutsiSetExerciseVideo, koutsiRemoveExerciseVideo, koutsiSeedExercises,
  koutsiCreateInviteCode, koutsiListInviteCodes, koutsiRevokeInviteCode,
  koutsiRedeemInviteCode, koutsiMatchUnclaimedPlayer, koutsiUnclaimedPlayersForCode, koutsiPlaceholderJoinCode, koutsiRedeemCoachKey, koutsiInviteLink, koutsiInviteMessage,
  koutsiCreatePlayer, koutsiBulkSetup, koutsiClaimPlayer,
  koutsiMyJoinCode, koutsiRotateJoinCode,
  ...
  koutsiSubmitFeedback, koutsiMarkWelcomeSeen, koutsiMarkCoachWelcomeSeen,
