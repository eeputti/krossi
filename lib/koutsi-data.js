...
function koutsiMapStudent(studentRow, profileRow, diaryRows, homeworkRows, videoRows, moodRows, matchNoteRows) {
  return {
    ...
    matchNotes: (matchNoteRows || []).filter((n) => n.student_id === studentRow.id).map((n) => ({ id: n.id, opponentName: n.opponent_name, date: n.date, note: n.note || '' })),
  };
}
...
async function koutsiLoadCoachState(coachId) {
  ...
  const [
    ...
    { data: diaryRows }, { data: homeworkRows }, { data: videoRows }, { data: moodRows }, { data: matchNoteRows },
  ] = await Promise.all([
  ...
  const students = (studentRows || []).map((s) => koutsiMapStudent(s, profileByStudentId.get(s.id), diaryRows || [], homeworkRows || [], videoRows || [], moodRows || [], matchNoteRows || []));
...
async function koutsiLoadStudentState(studentId) {
  ...
  const trainings = trainingRows.map((t) => koutsiMapTraining(t, absenceRows || []));
  const student = koutsiMapStudent(studentRow, profileRow, diaryRows || [], homeworkRows || [], videoRows || [], moodRows || [], matchNoteRows || []);
...
Object.assign(window, {
  ...
  koutsiCreateGroup, koutsiAddGroupMembers, koutsiCreateInviteCode, koutsiRedeemInviteCode, koutsiRedeemCoachKey,
  koutsiSaveGoal, koutsiSaveNote, koutsiSaveWish, koutsiAddMood, koutsiAddMatchNote,
