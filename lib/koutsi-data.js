...
function koutsiMapStudent(studentRow, profileRow, diaryRows, homeworkRows, videoRows, moodRows, matchNoteRows, goalRows) {
  return {
    ...
    homework: homeworkRows.filter((h) => h.student_id === studentRow.id).map((h) => ({ id: h.id, at: h.created_at, text: h.text, done: h.done })),
    ...
    moods: moodRows.filter((m) => m.student_id === studentRow.id).map((m) => ({ id: m.id, at: m.created_at, date: koutsiFmtShortDateFromTimestamp(m.created_at), score: m.score, note: m.note || '' })),
    ...
    goalHistory: (goalRows || []).filter((g) => g.student_id === studentRow.id)
      .map((g) => ({ id: g.id, at: g.created_at, goal: g.goal || '', previousGoal: g.previous_goal || '', byPlayer: g.changed_by === studentRow.id }))
...
async function koutsiLoadCoachState(coachId) {
  ...
  const [
    ...
    { data: diaryRows }, { data: homeworkRows }, { data: videoRows }, { data: moodRows }, { data: matchNoteRows }, { data: goalRows },
  ] = await Promise.all([
    ...
    studentIds.length ? koutsiSupabase.from('koutsi_goal_history').select('*').in('student_id', studentIds).order('created_at', { ascending: false }) : Promise.resolve({ data: [] }),
  ]);
  ...
  const students = (studentRows || []).map((s) => koutsiMapStudent(s, profileByStudentId.get(s.id), diaryRows || [], homeworkRows || [], videoRows || [], moodRows || [], matchNoteRows || [], goalRows || []));
...
async function koutsiLoadStudentState(studentId) {
  ...
  const [
    ...
    koutsiSupabase.from('koutsi_goal_history').select('*').eq('student_id', studentId).order('created_at', { ascending: false }),
  ]);
  ...
  const trainings = trainingRows.map((t) => koutsiMapTraining(t, absenceRows || []));
  const student = koutsiMapStudent(studentRow, profileRow, diaryRows || [], homeworkRows || [], videoRows || [], moodRows || [], matchNoteRows || [], goalRows || []);
...
// ── player-side writes ───────────────────────────────────────
// The goal lives on the student row (one current value) *and* is appended to
// koutsi_goal_history, so the Kehitys timeline can show what the goal used to be. The
// previous value is read back from the server rather than passed in, so a stale tab
// cannot record a wrong "aiemmin".
async function koutsiSaveGoal(studentId, goal) {
  const { data: current } = await koutsiSupabase.from('koutsi_students').select('goal').eq('id', studentId).maybeSingle();
  ...
  if (error) throw error;
  // History is a nice-to-have: a failed append must not lose the goal the player just set.
  await koutsiSupabase.from('koutsi_goal_history')
    .insert({ student_id: studentId, goal, previous_goal: previous || null, changed_by: studentId });
}
...
Object.assign(window, {
  ...
  koutsiAddMood, koutsiDeleteMood,
  koutsiAddMatchNote, koutsiUpdateMatchNote, koutsiDeleteMatchNote,
  koutsiSaveDisplayName, koutsiUploadAvatar, koutsiSaveCoachProfile,
